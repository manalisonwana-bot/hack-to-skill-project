...
export default async (req: Request, context: Context) => {
  ...
  if (pathname === "/api/calculate" && method === "POST") {
    try {
      ...
      const { personal, website } = body;
      if (!personal || !website) {
        return Response.json({ error: "Missing personal or website metrics" }, { status: 400 });
      }
  ...
  if (pathname === "/api/gemini/advisor" && method === "POST") {
    ...
    try {
      ...
      const { personal, website, metrics } = requestBody;
      if (!personal || !website || !metrics) {
        return Response.json({ error: "Missing report details" }, { status: 400 });
      }
    ...
    Return your response strictly as JSON that exactly matches this schema:
    {
      ...
      return Response.json({
        totalCarbonKg: requestBody?.metrics?.grandTotalKg || 5400,
        digitalCarbonKg: requestBody?.metrics?.digitalCarbonKg || 42.5,
        ecoScore: Math.round(75 + (requestBody?.metrics?.scores?.efficiency || 80) / 10),
        tips: [
          {
            category: "Digital",
            impact: "High",
